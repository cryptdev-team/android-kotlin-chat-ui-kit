...
class ErrorMessagesUtils {
    companion object {
        fun cometChatErrorMessage(context: Context?, e: String?) {
            when (e) {
                ERROR_INTERNET_UNAVAILABLE -> {
                    ...
                    showCometChatErrorDialog(
                        context,
                        context?.resources?.getString(R.string.something_went_wrong_please_try_again)
                    )
                }
            }
        }
        ...
        fun showCometChatErrorDialog(context: Context?, errorMessage: String?) {
            //Edited By CryptoDev: This function Casus a crash sometimes so surround it with try catch block
            try {
